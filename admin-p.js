...
const auth = getAuth(app); // අලුතින්
...
function initAuth() {
    ...
    $('loginForm')?.addEventListener('submit', async (e) => {
        ...
        const email = uname + "@admin.com"; // Username එක Email ආකෘතියකට හැරවීම

        const btn = e.target.querySelector('button');
        if (btn) btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> ලොග් වෙමින්...';

        try {
            // 1. Firebase Auth හරහා ලොග් වීම (මෙයට Database Rules අදාළ නැත)
            await signInWithEmailAndPassword(auth, email, pwd);

            // 2. දැන් Authentication සාර්ථක බැවින්, Database එක කියවීමට Rules වලින් අවසර ලැබේ
    ...
    $('btnLogout')?.addEventListener('click', async () => {
        try {
            await signOut(auth); // Firebase Auth එකෙනුත් Log out වීම
...
function applyRolePermissions() {
    if ($('adminRoleBadge')) {$('adminRoleBadge').innerText = currentAdminRole ? currentAdminRole.toUpperCase() : 'GUEST';
    }
...
function initNavigation() {
    $('openSidebar')?.addEventListener('click', () => {
        $('sidebar')?.classList.add('active');$('menuOverlay')?.classList.add('active');
    });

    const closeNav = () => {
        $('sidebar')?.classList.remove('active');$('menuOverlay')?.classList.remove('active');
    };

    $('closeSidebar')?.addEventListener('click', closeNav);$('menuOverlay')?.addEventListener('click', closeNav);
...
function initNewsModule() {
    if ($('newsDate') && !$('newsDate').value) {$('newsDate').value = new Date().toISOString().split('T')[0];
    }
            ...
            btn.addEventListener('click', async () => {
                try {
                    ...
                    if (docSnap.exists()) {
                        ...
                        $('newsCategory').value = d.category \vert{}\vert{} '';$('newsDate').value = d.date || '';
                        $('newsSnippet').value = d.snippet \vert{}\vert{} '';$('newsIsFeatured').value = d.isFeatured || 'no';
                        $('newsShowInHome').value = d.showInHome \vert{}\vert{} 'yes';$('newsReadMoreLink').value = d.readMoreLink || '';
...
// GOOGLE SHEET CSVමෙන්න ඔබ ඉල්ලූ පරිදි අලුත් Firebase Auth කොටස පමණක් වෙනස් කර, අනිත් සියලුම දේවල් කිසිදු වෙනසක් නොකර සකසන ලද සම්පූර්ණ JavaScript (JS) කේතය:
    ...
    $('newsForm')?.addEventListener('submit', async (e) => {
        ...
        try {
            if (fileInput && fileInput.files.length > 0) {
                ...
                showToast('ඡායාරූපය Upload වෙමින් පවතී...', 'ok');
                const res = await fetch(`[https://api.imgbb.com/1/upload?key=$](https://api.imgbb.com/1/upload?key=$){IMGBB_API_KEY}`, {
                    method: 'POST',
                    body: formData
                });
                const imgData = await res.json();
...
async function fetchAndParseGoogleSheet(sheetUrl) {
    ...
    const csvUrl = `[https://docs.google.com/spreadsheets/d/$](https://docs.google.com/spreadsheets/d/$){spreadsheetId}/gviz/tq?tqx=out:csv&sheet=mark%20sheet`;
...
function initDocumentsModule() {
    ...
    const links = [
        { label: 'Edit Text Book', url: '[https://docs.google.com/spreadsheets/d/1WZ13wzcv_Ca7u3ohBZnLgEUrs7h7wgQBo83-A-n4Ne0/edit?usp=drivesdk](https://docs.google.com/spreadsheets/d/1WZ13wzcv_Ca7u3ohBZnLgEUrs7h7wgQBo83-A-n4Ne0/edit?usp=drivesdk)', icon: 'fa-book' },
        { label: "Edit Teacher's Guide", url: '[https://docs.google.com/spreadsheets/d/1eOlkqUHWBo_PT9rc9IdwPaQtBWcBQ_pK2Z3g7XTqRXM/edit?usp=drivesdk](https://docs.google.com/spreadsheets/d/1eOlkqUHWBo_PT9rc9IdwPaQtBWcBQ_pK2Z3g7XTqRXM/edit?usp=drivesdk)', icon: 'fa-chalkboard-user' },
        { label: 'Edit Short Notes', url: '[https://docs.google.com/spreadsheets/d/1FuK5JY1SP33OOrACsoFxC5WyZYqbyj_Q4-K4C54TEv4/edit?usp=drivesdk](https://docs.google.com/spreadsheets/d/1FuK5JY1SP33OOrACsoFxC5WyZYqbyj_Q4-K4C54TEv4/edit?usp=drivesdk)', icon: 'fa-note-sticky' },
        { label: 'Edit Application', url: '[https://docs.google.com/spreadsheets/d/19zyl4ZulZc8iTbodIEosWaJlZgFLn3SVqIomV_9tP5o/edit?usp=drivesdk](https://docs.google.com/spreadsheets/d/19zyl4ZulZc8iTbodIEosWaJlZgFLn3SVqIomV_9tP5o/edit?usp=drivesdk)', icon: 'fa-file-lines' }
    ];
